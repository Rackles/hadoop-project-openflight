...
records = LOAD '$input' using PigStorage('\t')  AS (date:chararray, time:chararray, xloc:chararray, sc:int, c-ip:chararray, cs-m:chararray, cs:chararray, cs-uri-stem:chararray, sc-s:chararray, csR:chararray, csU:chararray, csq:chararray, csC:chararray, x-edge-result-type:chararray, x-edge-request-id:chararray);
